...
def load_data(data_path, output=True):
    """
    Function to load data from a directory of CSV files and concatenate them into a single DataFrame.
    Optionallly prints the percentage of each label in the dataset.
    """
...
def balanced_sample(data, target_column, fraction, random_state=42):
    """
    Function to balance the dataset by sampling the desired number of examples from each label.
    The dataset reduced to the fraction of the original size and resampled to have equal numbers of each label except for label 0 which has twice as many.
    Where there aren't enough values, the remaining samples are taken from the smallest available datasets.
    """
...
def create_FCNN_loader(inputs, targets, device, batch_size=128, shuffle=True, num_workers=1, standardise_scalar=None, normalise_scalar=None, scale_targets=False):
    """
    Create a FCNN DataLoader for the given dataset.
    """
...
def gen_rand_data(npoints=100, rand_state=42):
    ...
    # Diviner: Scale to range 35 to 350
    ...
    # LOLA, M3, Mini-rf: Scale to range 0 to 1
...
def stratified_split_data(features, targets, rand_state=42):
    ...
    Falls back to regular random splits if stratified splitting is not feasible.
    """
    torch.manual_seed(rand_state)
...
def get_random_filtered_graph(file_path, label_value=None, k=5):
    """
    Function to get a random subgraph from the dataset with k*k nearest neighbours.
    """
